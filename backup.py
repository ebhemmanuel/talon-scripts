...
class UserActions:
    def backup_and_commit_scripts(source: str, destination: str):
        ...
        try:
            ...
            script_content = f'cd "{source_dir}"\ngit add .\ngit commit -m "{commit_message}"\n'
            script_path = os.path.join(source_dir, "temp_git_commands.sh")
            with open(script_path, "w") as script_file:
                script_file.write(script_content)
            
            # Open Git Bash and execute the script
            git_bash_path = r"C:\Program Files\Git\git-bash.exe"
            subprocess.Popen([git_bash_path, '--cd=' + source_dir, '-c', f'bash {script_path}'])
